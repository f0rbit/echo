...
import { chaser_c, dir_c, path_c, player_c, state_c } from "../../components.ts";
...
export const chaser_step_system: System = (w, ctx) => {
	const arena = ctx.res.get(arena_r);
	if (!arena.ok) return;
	const { floors } = arena.value;
	const blocked_by = (c: Cell): boolean => !floors.has(g.key(c.x, c.y));

	for (const [id, p, pth] of w.query([pos_c, path_c, chaser_c] as const).collect()) {
