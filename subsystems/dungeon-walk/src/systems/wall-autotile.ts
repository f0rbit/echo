...
const PATTERN_TO_TILE: Readonly<Record<number, { col: number; row: number }>> = {
	...
	0b0010: { col: 8, row: 0 },   // E only -> left-cap of horizontal (outer NW corner)
	0b0011: { col: 6, row: 0 },   // N+E -> inner corner, floor on SW
	0b0100: { col: 0, row: 0 },   // S only -> top-cap of vertical
	0b0101: { col: 0, row: 1 },   // N+S -> vertical run middle
	0b0110: { col: 8, row: 0 },   // S+E -> outer top-left corner (room outline)
	0b0111: { col: 8, row: 1 },   // N+E+S -> T-opens-W (right edge of room)
	0b1000: { col: 11, row: 0 },  // W only -> right-cap of horizontal (outer NE corner)
	0b1001: { col: 5, row: 0 },   // N+W -> inner corner, floor on SE
	0b1010: { col: 9, row: 0 },   // E+W -> horizontal run middle
	0b1011: { col: 9, row: 0 },   // N+E+W -> T-opens-S (top edge of room)
	0b1100: { col: 11, row: 0 },  // S+W -> outer top-right corner (room outline)
	0b1101: { col: 11, row: 1 },  // N+S+W -> T-opens-E (left edge of room)
	0b1110: { col: 9, row: 3 },   // E+S+W -> T-opens-N (bottom edge of room)
