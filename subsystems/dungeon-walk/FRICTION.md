...
> **Status (post-v0.2.0)**: every promotable entry has been resolved by forge v0.2.0; this doc is now historical. The migration cut `subsystems/dungeon-walk/src/**/*.ts` from 465 → 357 LOC (-108).
...
**Stays game-side** per planner verdict (option c — delta-tracking in the consumer). Not yet adopted in dungeon-walk; the post-v0.2.0 fov system still does a full sweep but at least benefits from `query_data` dropping the marker slot. Revisit if perf becomes an issue or a second consumer hits the same shape.
...
**RESOLVED in forge v0.2.0** — `world.query_data(data, markers)` ships in core; markers are dropped from the yielded tuple. dungeon-walk's systems now use `query_data([pos_c] as const, [floor_c] as const)` etc. Note: the `as const` is needed under `noUncheckedIndexedAccess` to keep the destructure tight — minor friction worth a v0.2.x doc note.
...
**RESOLVED in forge v0.2.0** — `presets.movement_4way_digital` ships discrete 4-way bindings (no axes, deadzone 0). dungeon-walk still uses `movement2d` for now since the existing input-axis path stays consistent with the recorded replay; switching to the digital preset is a follow-up worth doing once we want per-tap stepping.
