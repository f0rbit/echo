...
//   Consumers: arena (subsystems/arena/src/systems/particles.ts),
//              progress (this file).
//   Planned promotion: Phase 8 `main` -> `@f0rbit/forge/fx/particles`.
//
// Cell-step progress copy of arena's ring-buffer particle pool.
// Constants tuned for the cell-step kill burst (more particles, faster
// decay, smaller spread) so a single Z-press kill reads as a punchy
// burst at the chaser cell rather than arena's larger spray. Gate
 ...
 * fixed-capacity array (256 by default); `head` wraps when emit pushes
 * past capacity.
 *
 * Three systems:
 *  - emit (update): on each hit event, write PARTICLES_PER_HIT entries
 *    with seeded jitter. Overwrites stale entries when full.
 *  - advance (update): integrate position by velocity * fixed_dt;
 *    decrement ttl. Pure data — no rendering.
 *  - render (post): draw entries with `ttl > 0` to a Graphics overlay
 *    added to `app.render.world` in main.ts. Fade alpha by `ttl /
 *    max_ttl`.
 */
...
// Emit is NOT gated on hitstop: hits and particle emission happen on
// the same tick that hitstop_trigger sets remaining > 0. Gating here
// would mean particles never emit for a kill burst, because
// hit_events_r is cleared in `pre` at the start of the next tick
// (before hitstop releases). `particles_advance` IS gated so emitted
// particles freeze in place during the hitstop "punch" — that's the
// visual stop. Mirrors arena's pattern exactly.
