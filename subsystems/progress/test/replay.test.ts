...
// seed=1). Re-recorded after the movement-reads-live-input fix — the
// player no longer slides forever on a single keypress; movement now
// stops when the live axis returns to 0. That shifts spawn / pathing /
// kill timing, which reshuffles the perk-choice RNG pre-images. Final
// perks now: {xp_gain, hp_plus, xp_gain} (note xp_gain stacks twice).
// Player still survives all 3 picks; level still hits 4.
const EXPECTED_FINAL_PERKS: readonly string[] = ["perk.xp_gain", "perk.hp_plus", "perk.xp_gain"];
// BASE stats { atk: 5, def: 2, spd: 1, max_hp: 10, xp_gain_mul: 0 } plus
// perk.xp_gain (+0.25 xp_gain_mul) x2 + perk.hp_plus (+5 hp):
//   atk = 5
//   def = 2
//   max_hp = 10 + 5 = 15
//   spd = 1
//   xp_gain_mul = 0 + (1.25 * 1.25 - 1) = 0.5625  (multiplicative stack)
const EXPECTED_FINAL_STATS = {
	atk: 5,
	def: 2,
	spd: 1,
	max_hp: 15,
...
// Recorder reaches level 4 + 3 picks at tick 1382, then settles 30 ticks.
const EXPECTED_END_TICK = 1412;
// Tick after the first level-up pick (pick processed at tick 305).
const TICK_AFTER_FIRST_PICK = 306;
...
// after the third level-up (tick 1382) but before settle ends at 1412.
const MID_REPLAY_SNAP_TICK = 1400;
